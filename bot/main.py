...
async def on_message(message):
    ...
    if '!help'.lower() in message.content.lower() and message.content[0] == "!":
        await message.channel.send("Use !open [mega21/mega22]. Always assumes no ships are currently owned and will not account for duplicates and pity successes when opening one container after another.")
    if '!open'.lower() in message.content.lower() and message.content[0] == "!":
        containerData = containerDataList.index(difflib.get_close_matches(message.content.lower().partition(' ')[2], containerNameList, n=1, cutoff=0)[0]) #finds closest container name to input
